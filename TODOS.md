...
- [ ] **`盈利能力`-section schema drift silently yields `roe=None`** — `akshare_filing.py::_profitability_metric` uses `df.get("选项")`/`df.get("指标")`; a renamed section (`盈利能力`→…) makes the mask all-False → `roe=None` with NO `failure_reason` appended and no log, indistinguishable from "legitimately absent". Matches the existing `_common_metric` convention and the spec ("ROE absence never fails the digest"), so acceptable — but means AkShare section-drift is unobservable in production logs. Consider a structured debug log on the swallow (same family as the item-001 `fetch_cn_index_valuation` broad-except TODO). (funding-analysis-004 ship silent-failure review 2026-05-31)
