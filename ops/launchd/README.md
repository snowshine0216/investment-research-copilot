...
| `com.irc.flow-capture` | Daily 15:45 (after the 15:00 close) — **never run manually before the close, the manual path is unguarded** | `irc monitor flow-capture` → `irc rotation` (sector rotation radar, ADR 0023, chained after capture) | skips weekends + CN holidays; single-instance lock `.flow-capture.lock`; best-effort **data-health notify** (`notify-status --run-kind flow-capture --no-notify-on-clean`): silent on a fully-ok chain, pages on rotation `abstain`/`degraded_*` (tagged with a `连续第 N 日` consecutive-day counter) or a capture failure, fires a one-time `轮动雷达恢复 ok` recovery notice on the abstain→ok transition, also pages `degraded` with `flow-capture: N/M` when today's row covers less than 80% of the flow store's union symbols (even at wrapper rc=0); a capture timeout (rc=124) pages `failed`; the rotation step is advisory-only (failure/abstain surfaces via the notify tail, but never changes the wrapper rc) |
