...
| `MINIMAX_API_KEY`, `MINIMAX_BASE_URL`, `MINIMAX_MODEL` | Required for `irc monitor` | MiniMax provider for monitor narrative tasks (`monitor_impact`, `monitor_narrative`). Validated at the call edge; legacy `irc run` tasks do not need these. |
