...
- *Alternative — strip inline `[N]` citation markers from the excerpt.* Rejected: the 3 themes that already read well today carry inline `[N]` markers in the rendered output. Stripping would change their summary content → change their citation_id → expand the F5 churn surface needlessly without semantic gain. The default is "leave them in".
