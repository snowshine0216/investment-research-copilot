...
def _upsert_prices(con, instrument_id: str, ticker: str, df: pd.DataFrame) -> int:
    ingested_at = _now_iso()
    for r in df.itertuples(index=False):
        ref = build_ref_id("openbb", "prices", instrument_id, str(r.date))
        con.execute(
            """
...
def _upsert_macro(con, series_id: str, df: pd.DataFrame) -> int:
    ingested_at = _now_iso()
    for r in df.itertuples(index=False):
        ref = build_ref_id("openbb", "macro_series", series_id, str(r.date))
        con.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            [series_id, str(r.date), float(r.value), ingested_at, "openbb", ref],
        )
    return len(df)
...
def _upsert_nav(con, instrument_id: str, df: pd.DataFrame) -> int:
    ingested_at = _now_iso()
    for r in df.itertuples(index=False):
        ref = build_ref_id("akshare", "nav_history", instrument_id, str(r.date))
        con.execute(
            """
...
def run_ingest(repo_root: str) -> int:
    ...
    try:
        ...
        ob_counts: dict[str, int] = {"prices": 0, "macro_series": 0}
        ...
        for instr in all_price_instruments:
            ...
            ob_counts["prices"] += _upsert_prices(con, instr.instrument_id, instr.ticker, df)
