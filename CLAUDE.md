...
uv run irc monitor                   # daily brief for the 10-fund Monitor set → outputs/<date>/monitor/{report.html,drilldown.html,eval_trace.json}; appends data/monitor/forward_ledger.jsonl. active_cn_equity funds get a capital-flow factor + a bottom-up dual-track valuation factor (per-stock self-history + industry-relative + False-Cheap clamp) + per-stock PB/PE+industry+flow drill-down (ADR 0019 / 0020).
