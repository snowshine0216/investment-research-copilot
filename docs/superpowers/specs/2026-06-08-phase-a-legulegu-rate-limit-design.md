...
behaviour**: pace so the burst detector never trips, retry the transient 504 with a
cooldown long enough for an escalated limiter to recover, and **stop the whole sweep
once a sustained cooldown is confirmed** rather than poking all 8 calls.
...
Exception: `class LeguleguCooldownExhausted(Exception)` — the provider-wide
suspension signal.
...
- **throttle exhausted → raises `LeguleguCooldownExhausted`** (provider-wide; the
...
- The **csindex** sector call stays on plain `_fetch_frame` — different host, out of
  scope; we do not pace the 14-slug sector leg.
...
log, `break`, and write whatever landed before the trip:
...
for key in index_keys:
    try:
        ...
        _log.warning("legulegu cooldown exhausted at %s; suspending remaining "
                     "broad-leg keys this run", key)
        break
    ...
```

A fully-throttled sweep now makes ~2 calls then stops — not 16. The sector leg
...
- keep `test_replace_keys_skips_key_when_fetch_lacks_pe_ttm` (PB-only) and **add** the
  inverted PE-only case (all `pb=None`) → 0 rows written, cache untouched, WARNING
  naming the `pb` axis.
...
1. **Recover → Gate #4 (alone):** `IRC_RUN_LIVE_AKSHARE=1 uv run pytest -m live_akshare
   tests/fundamentals/test_index_valuation_live.py -v -s` → 4 passed (sweep stays
   skipped without `IRC_RUN_LEGULEGU_SPECULATIVE=1`). If any symbol returns `None` the
   limiter is hot → **stop, wait for full recovery**, and only then consider raising
   `_LEGULEGU_GAP_S` and re-running. Never re-run into an active cooldown.
