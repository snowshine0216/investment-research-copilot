...
- Ship step-8 review round (silent-failure-hunter, P0) — `_build_flow_capture_health` returned `cov=None` on a missing/corrupt flow store, which made `rotation_health` skip the coverage check entirely and emit no health item at all, contradicting §3.3 ("unreadable → health_unknown item"). Fixed conservatively at the edge: merge `health_unknown()` into the returned digest when `flow is None`, keeping the radar-derived items and recovery notice untouched. Added `test_flow_capture_missing_flow_store_is_unknown`; updated the three pre-existing flow-capture tests to seed a date-rolled, production-shaped `fund_flow_series.json` (via a new `_flow_store_rolled_to`/`_seed_flow_store` helper pair) instead of leaving the store unseeded, so their abstain/recovery/silent scenarios stay pure rather than incidentally exercising the missing-store path. No existing assertion weakened; `tests/commands/test_notify_cmd.py` 38 passed, `tests/notify/` 91 passed (unchanged), ruff clean.
