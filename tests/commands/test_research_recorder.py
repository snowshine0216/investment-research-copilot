...
@pytest.fixture
def research_repo_with_tavily(tmp_path: Path) -> Path:
    """Repo with Tavily wallet entry so the ledger decrement can land."""
    (tmp_path / "config").mkdir(parents=True)
    (tmp_path / "config/spend_pricing.yaml").write_text(
        yaml.safe_dump({
            "margin": 1.2,
            ...
            "search": {
                "tavily": {"currency": "credits", "per_query": 2.0},
            },
            "search_seeds": {
                "tavily": {"units": 40},
            },
    ...
    (tmp_path / "config/spend_balances.yaml").write_text(
        yaml.safe_dump({
            "tavily": {"balance": 1000, "as_of": "2026-06-01"},
        }),
        encoding="utf-8",
    )
    (tmp_path / "outputs" / "2026-06-06").mkdir(parents=True)
...
def test_research_run_records_research_synth_samples_0_to_1(research_repo, monkeypatch):
    ...
    # Stub run_research_pipeline to return (rc, cost_entries, search_units) — 3-tuple
    with patch("irc.commands.research_cmd.run_research_pipeline",
               return_value=(0, [fake_cost_entry], {})):
...
def test_research_search_units_flow_to_actuals_and_ledger(
    research_repo_with_tavily, monkeypatch
):
    """§15.2 ledger-box proof: search_units from run_research_pipeline land in
    spend_actuals.json AND decrement consumption.json for a provider with a balance.

    TDD: this test FAILS against search_units={} (pre-fix) and passes once the
    pipeline returns real counts and research_cmd threads them through.
    """
    monkeypatch.setenv("IRC_SKIP_SPEND_GATE", "1")
    ...
    # Pipeline returns real search_units: tavily did 3 query calls, jina did 2 pages
    fake_search_units: dict[str, int] = {"tavily": 3, "jina": 2}

    with patch(
        "irc.commands.research_cmd.run_research_pipeline",
        return_value=(0, [], fake_search_units),
    ):
        with patch("irc.commands.research_cmd.build_providers", return_value=(object(),)):
            with patch("irc.commands.research_cmd.build_extractor", return_value=object()):
                rc = research_mod.run_research(str(research_repo_with_tavily))

    assert rc == 0

    # (a) spend_actuals.json carries the search_units
    actuals_path = (
        research_repo_with_tavily / "outputs" / "2026-06-06" / "spend_actuals.json"
    )
    assert actuals_path.exists(), "spend_actuals.json not written"
    actuals = json.loads(actuals_path.read_text())
    assert actuals.get("search_units", {}).get("tavily") == 3, (
        f"expected search_units.tavily=3, got {actuals.get('search_units')}"
    )
    assert actuals.get("search_units", {}).get("jina") == 2

    # (b) consumption.json decremented for tavily (wallet entry present)
    cons_path = research_repo_with_tavily / "data" / "spend" / "consumption.json"
    assert cons_path.exists(), "consumption.json not written — ledger decrement missing"
    cons = json.loads(cons_path.read_text())
    assert cons.get("tavily", {}).get("consumed_since") == pytest.approx(3.0), (
        f"tavily ledger not decremented: {cons.get('tavily')}"
    )
