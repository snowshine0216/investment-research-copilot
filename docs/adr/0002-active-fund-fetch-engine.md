...
**F6 QDII premium-to-NAV fetcher (item 002, 2026-05-26):** `fetch_qdii_premium_pct` follows the same fund-level fetch pattern as `_build_fund_level_snapshot` — a `lru_cache(maxsize=1)`-decorated bulk-table accessor (`_fetch_full_etf_spot_table`) keyed on `()` so all QDII symbols in one run share a single AkShare call, with degrade-to-`None` on exception / empty / schema drift / missing-symbol. The fetcher's output is a scalar score-row field (`qdii_premium_pct`), not a `ThesisEvidence` record — it feeds the decision-stage `qdii_premium_too_high` gate, not the dual-coverage gate (the QDII fetch reform of 2026-05-25 already handles the dual-coverage side via fund-level NAV + announcements). Off-exchange QDII feeders skip the fetcher and get `qdii_premium_pct=0.0` injected at the scoring boundary (off-exchange units transact at NAV by construction). See `CONTEXT.md` "QDII premium-to-NAV" section and `docs/2026-05-26-decision-confidence-followup/items/002-spec.md`.
