...
1. **AC1 — New gap code defined.** ~~A new `OpportunityRow.evidence_gaps` code `top_holdings_broker_thin` is added to the project's gap-code vocabulary.~~ — corrected by grill: the new code is an `advisory_gaps` member, NOT an `evidence_gaps` member (see AC6 + ADR 0005). It is documented in CONTEXT.md "Failure-mode + audit policy" alongside the new `advisory_gaps` glossary entry; the existing `missing_broker_coverage` entry stays unchanged as the legacy blocking gap on `ConstituentSnapshot`. ADR 0005 records the rationale for the new field (decision is hard-to-reverse: touches frozen dataclass, serializer, card mirror, three renderer sites + lockdown).
...
6. **AC6 — H3 invariant preserved.** ~~Adding `top_holdings_broker_thin` to `evidence_gaps` MUST NOT route the row to `gapped_rows` in `_write_opportunity_outputs`'s H3 partition.~~ — corrected by grill: the code never enters `evidence_gaps` at all; see Q7 resolution. `derive_thesis_from_evidence` returns the code in its flat `gaps` slot; `_partition_gaps` in `states.py` becomes a 3-way split returning `(real_gaps, expected_omissions, advisory_gaps)` and routes `top_holdings_broker_thin` into the new `OpportunityRow.advisory_gaps: tuple[str, ...] = ()` field. The H3 partition predicate stays exactly `evidence_gaps == ()` — unchanged. `ADVISORY_GAP_CODES: frozenset[str] = frozenset({"top_holdings_broker_thin"})` lives at module-top in `states.py` next to `EXPECTED_OMISSION_CODES`. ~~(or, equivalently, the existing `expected_omissions` field if its semantic can be widened — see Open Q below)~~ — corrected by grill: rejected; `expected_omissions` semantic is "structural non-feature by design" and conflating with "actionable advisory" would break every downstream consumer's discrimination. See ADR 0005. **Locked by AC test:** every row that carries `top_holdings_broker_thin` also satisfies `row in publishable_rows` of the H3 partition.
...
8. **AC8 — Pick-ordering demotion in §5 picks table.** ~~`_build_pick_rows` in `commands/memo_cmd.py` (or wherever pick ordering is established for §5) appends a tiebreaker: when two rows have equal upstream priority, the one carrying `top_holdings_broker_thin` sorts AFTER the one without. The primary sort key is unchanged; this is a **stable secondary key**.~~ — corrected by grill: `_build_pick_rows` does not sort today; it preserves `trade_plan.yaml` iteration order. There is no "primary sort key" to layer a secondary on. Replacement: after `_build_pick_rows` produces `pick_rows`, apply a **stable partition** — `[r for r in pick_rows if "top_holdings_broker_thin" not in r.advisory_gaps] + [r for r in pick_rows if "top_holdings_broker_thin" in r.advisory_gaps]` — preserving trade-plan iteration order within each partition. `PickRow` gains an `advisory_gaps: tuple[str, ...] = ()` field (default empty) populated from `op.get("advisory_gaps") or ()` in `_build_pick_rows`. Locked by a unit test feeding two synthetic rows with identical upstream order: the gap-bearing row appears later in the rendered picks-table.
...
Grill-with-docs pass run autonomously on 2026-05-27 by autodev. All questions auto-accepted the recommended answer. Recommendations stress-tested against `CONTEXT.md`, `src/irc/opportunity/{types.py,states.py,thesis_evidence.py,policy_b.py,report.py}`, `src/irc/commands/memo_cmd.py`, and ADR 0003. Each row below: question → auto-accepted answer → rationale → doc impact.

- **Q-A1: Does the spec's proposed `OpportunityRow.advisory_gaps` collide with the existing `evidence_gaps` semantics?**
  **A:** No collision IF the field is dedicated (not folded into `evidence_gaps`). Recommended: new field, NOT widened `evidence_gaps`. Rationale: H3 partition predicate (ADR 0003 §3) is `evidence_gaps == ()`; any non-empty value routes to `gapped_rows`. The 4-field failure renderer contract is the enforcement mechanism. Folding advisory codes into `evidence_gaps` would require a publishable-safe allowlist distributed across `_write_opportunity_outputs`, `_partition_gaps`, and the discipline bucket sections — three places where the next contributor could "fix" the allowlist by removing an entry and silently shift rows between buckets. Doc impact: **ADR 0005** + CONTEXT.md (`advisory_gaps` entry; H3 entry sentence on orthogonality).

- **Q-A2: Should the H3 invariant predicate widen to include `advisory_gaps`?**
  **A:** No. The H3 predicate stays exactly `evidence_gaps == ()`. `advisory_gaps` is orthogonal and carried through into publishable outputs. Rationale: widening would invert H3's load-bearing semantics ("any gap → failure section") into a code-by-code allowlist — fragile and surprising. Doc impact: CONTEXT.md H3 entry gets a sentence stating the predicate is unchanged and advisory_gaps does NOT participate.

- **Q-A3: Should `expected_omissions` widen to include `top_holdings_broker_thin`?**
  **A:** No. `expected_omissions` is scoped to "structural non-features by design" (e.g. `constituent_not_applicable` for asset classes with no constituents). `top_holdings_broker_thin` is the opposite — it's a real, actionable evidence gap that happens to not warrant fail-close. Conflating the two would force every consumer (cards, report serializer, renderer, audit gate) to disambiguate via per-code branching. Doc impact: ADR 0005 considered-options + CONTEXT.md `advisory_gaps` entry.

- **Q-A4: Should `top_holdings_broker_thin` live in CONTEXT.md "evidence-gap catalog" or similar?**
  **A:** Yes — added next to `audit_errors` in the "Failure-mode + audit policy" section, alongside the new `advisory_gaps` field entry. The existing `missing_broker_coverage` entry (legacy `ConstituentSnapshot` blocking gap) stays unchanged; the two are explicitly distinguished as "blocking vs advisory." Doc impact: CONTEXT.md update done.

- **Q-A5: Does the design actively preserve the `thesis_state` setter invariant (not just "fails to violate")?**
  **A:** Yes — the gap is emitted by `derive_thesis_from_evidence`'s existing active-fund branch through the existing `gaps` return slot (position 4 of the 5-tuple). `thesis_state` (position 0) is not touched. No new setter is introduced; `build_opportunity_row` still calls `_partition_gaps` on the flat gap list to route real-gaps vs expected-omissions vs advisory-gaps. Rationale: the invariant says "`thesis_state` is set ONLY by `derive_thesis_from_evidence`" — by going through that exact function's gap return, the design actively reinforces the locus of authority. Doc impact: none (already covered).

- **Q-A6: Is the OR semantics of the threshold (≥2 of Top-5 OR ≥20% summed weight) surprising vs codebase precedent?**
  **A:** Mildly surprising (no OR precedent — `FOREIGN_HEAVY_THRESHOLD` is a single `>=` comparator; `MATERIAL_HOLDING_QUORUM` is `ceil(top_n/2)`). But the rationale holds: count-of-2 alone misses the 1-of-5 / 25%-weight case; weight alone misses the 2-of-5 / 5%-weight-each case where the manager has scattered concentration. Both branches are deterministic and unit-testable in isolation. Constants are named `Final` (`TOP_HOLDINGS_BROKER_THIN_COUNT_THRESHOLD = 2`, `TOP_HOLDINGS_BROKER_THIN_WEIGHT_PCT_THRESHOLD = 20.0`) so the OR is named, not hidden. Doc impact: none for ADR (not surprising enough to warrant one); spec AC3 already captures.

- **Q-A7: Pick-ordering tiebreaker — where in the ordering hierarchy?**
  **A:** Not a tiebreaker. `_build_pick_rows` does NOT sort today; it preserves `trade_plan.yaml` iteration order. The correct construct is a **stable partition** applied after `_build_pick_rows`: `pick_rows = [r for r in pick_rows if not r.advisory_gaps_blocking] + [r for r in pick_rows if r.advisory_gaps_blocking]`. Within each partition, trade-plan order is preserved. Locked by a unit test. Doc impact: spec AC8 corrected via strike-through.

- **Q-A8: Active-fund-only scope — how is "active fund" detected?**
  **A:** Two signals exist in the codebase: `_is_active_fund(OpportunityInput) → bool` in `states.py` (uses `asset_class == "cn_equity_fund" and market != "cn_on_exchange"`) and `isinstance(snapshot, ActiveFundSnapshot)` in `thesis_evidence.py`. The gap-emission path goes through `derive_thesis_from_evidence`'s `ActiveFundSnapshot` isinstance branch, which is the natural gate — `FundLevelSnapshot` and `ConstituentSnapshot` branches never call the new helper. AC4 already aligns. Doc impact: none.

- **Q-A9: Where does the gap-emission helper live without enlarging the existing `thesis_evidence.py` violation (454 lines)?**
  **A:** New sibling helper file `src/irc/opportunity/advisory_gaps.py` (≤80 lines): `count_broker_empty_top5`, `weight_broker_empty_top5`, `should_emit_top_holdings_broker_thin`, `TOP_HOLDINGS_BROKER_THIN_*` constants, `ADVISORY_GAP_CODES: frozenset[str]` (re-exported from `states.py` for `_partition_gaps`). `thesis_evidence.py`'s active-fund branch imports and calls `should_emit_top_holdings_broker_thin(snapshot)` — a 1-line addition. Doc impact: spec planning note (already implied by constraints).

- **Q-A10: Does adding `advisory_gaps` warrant an ADR by the three-of-three rule?**
  **A:** Yes — adopted ADR 0005.
  - Hard to reverse: frozen dataclass + serializer + card mirror + three renderer sites + item 008 lockdown fixture.
  - Surprising without context: a third gap-like tuple field next to `evidence_gaps` and `expected_omissions` — future readers will reach for one of the existing two without ADR 0005 explaining why.
  - Real trade-off: three placements considered (fold / widen / new); each rejected option had a concrete failure mode (H3 inversion, semantic conflation).
  Other proposed decisions (OR threshold, stable partition, helper extraction) fail at least one of the three and do NOT receive ADRs. Doc impact: ADR 0005 created.

- **Q-A11: Foreign-heavy interaction — both Policy B rule 2.5 acceptance AND `top_holdings_broker_thin` may fire on the same row. Contradiction?**
  **A:** No — both are correct simultaneously and represent different concerns. Rule 2.5 accepts the row on fund-level evidence (CN-registered vehicle with HK/US holdings; per-holding CN broker pipeline didn't reach those tickers). `top_holdings_broker_thin` warns the operator that per-holding evidence depth is structurally thin — exactly the warning the operator wants when ranking foreign-heavy funds against fully-covered domestic peers. Doc impact: CONTEXT.md `advisory_gaps` entry calls out coexistence with rule 2.5.

- **Q-A12: Citation gate v1 dual-leg structural binding — affected?**
  **A:** No. `advisory_gaps` does not change `OpportunityRow.thesis_evidence` shape, citation_id format (`\[ref:[0-9a-f]{16}\]`), or the data/information leg structural binding. `IRC_CITATION_ENFORCE_MODE=block` still passes for the same set of rows. Doc impact: none.

- **Q-A13: SAME-3 invariant — affected?**
  **A:** No. The new gap does not add to `thesis_evidence`; the picks-table 证据 cell, evidence-pool refs, and discipline `_render_section` refs read `thesis_evidence` only, unchanged. Doc impact: none.
