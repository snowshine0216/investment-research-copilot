...
`push2/ulist.np/get?secids=<our list>&fields=f12,f14,f184,f100` returns each
secid's today 主力净流入净占比 (f184, percent-points) AND its 东财行业 name
(f100) in ONE request — no per-symbol throttle. Routed through IRC_CN_PROXY at
...
Percent-points, NO /100. Returns (flow, industry) maps; 行业 batch-first per
ADR 0020 addendum 2026-07-03.

FIELD-CODE NOTE (probe 2026-07-04): EastMoney push2 field numbers are
INTERFACE-SPECIFIC. In `stock/get` (per-symbol em_raw fallback) and the `clist`
fund-flow interface, f127 = 行业. But in `ulist.np/get`, f127 is a NUMERIC field
(涨速/change-rate) and 行业 rides on f100 — so the batch reads f100, while the
per-symbol stock/get fallback keeps its own (correct) f127. Requesting f127 here
returned a number for every symbol → _coerce_industry rejected it → 0/60 fill."""
...
def _coerce_industry(value: object) -> str | None:
    """Pure: f100 → stripped non-empty industry string, else None (missing key /
    '-' / '' / whitespace / non-string → None — never fabricated). Non-string
    guard also rejects f-code drift where a numeric field lands where 行业 was
    expected (the f127→f100 root cause, probe 2026-07-04)."""
...
def parse_ulist(payload: dict) -> dict[str, tuple[float | None, str | None]]:
    """Pure: {f12 → (f184 percent-points NO /100, f100 行业 | None)}. Tolerant of
    list/dict diff shape. Blank/missing data → {} (→ all None upstream, never
    fabricated). 行业 is f100 in ulist.np — NOT f127 (numeric here; see module
    docstring)."""
    ...
    return {str(r.get("f12")): (_coerce(r.get("f184")), _coerce_industry(r.get("f100")))
            for r in rows}
...
def fetch_flow_today_batch(
    ...
    params = {"ut": _UT, "fltt": "2", "invt": "2", "np": "1", "dect": "1",
              "secids": build_secids(symbols), "fields": "f12,f14,f184,f100"}
    ...
    for s in dict.fromkeys(symbols):
        f184, ind = by_symbol.get(s, (None, None))
        flow[s], industry[s] = f184, ind
