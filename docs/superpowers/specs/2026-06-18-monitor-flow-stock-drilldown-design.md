...
- `render_drilldown.py` (PURE):
  ...
  - `flow_rollup_html(metrics, agg, signal)`: the reconciliation line — `flow factor = Σ(wᵢ·sᵢ)/Σ(wᵢ) = <value> (covered <ratio>% of top-5; **top-5 = <Σ weight_pct>% of fund AUM**)`, then how `valuation` + `flow` contributions land in `C` → bias. The "top-5 = X% of fund AUM" context (sum of the top-5 `weight_pct`) is **always shown** so an ADD leaning on flow carries its representativeness (a 28%-of-AUM read is discountable vs. a 55% one) — transparency, not a coverage veto. This is the "dig to the bottom" methodology made explicit.
...
- **Framing (the ADR 0015 line — grill 2026-06-18):** bias stays "研究参考信号，非买卖指令" (#156). The drill-down board explains *why* the lean leans (PB/PE + flow) using lean language (偏多/偏空); it MUST NOT cross into `portfolio_action` territory — **no imperative 买入/卖出, no target weights, no current-vs-target weight deltas, no per-instrument action**. Real `portfolio_action ∈ {buy, trim_review, exit_review, review}` with weight deltas is ADR 0015's contract in the **decision layer** on **held** positions; monitor's better-grounded lean does not promote into it. "Add-bias = buy signal" means a better-*reasoned* lean, never an executable order.
...
## 9. Out of scope (YAGNI) / staged follow-ups

- **Dual-track per-stock valuation + "False Cheap" guard — STAGED to its own spec (grill 2026-06-18).** This spec scores per-stock valuation by **self-history percentile only**; the board shows raw PE/PB alongside it. The follow-up adds: a **60/40 blend** of self-history percentile + a **cross-sectional industry-richness** leg, plus a **False-Cheap clamp** (cheap-vs-self BUT rich-vs-industry → that stock's valuation score is clamped to `0`/NEUTRAL). Locked design corrections for that spec: (1) the cross-sectional leg is "stock PE vs **industry-average** PE" (Option A — two cacheable akshare tables + a stock→industry map), NOT a true within-peer percentile (Option B, rejected as a heavy per-peer fetch with ADR-0014 rate-limit exposure) and NOT a sector-blind absolute ceiling (Option D, rejected); (2) **drop "DCF/PEG"** — monitor computes neither, and the existing `valuation_fundamental_signal` (ADR 0012) is fund-level + dormant (`consensus_upside_pct=None`, ADR 0009), so it cannot serve as the per-stock guard; (3) the clamp is a **per-stock score adjustment that flows through the existing linear composite**, NOT a fund-level veto bolted onto the linear signal model. **Conflict-suppression** belongs to this same staged veto-class work: in this spec `valuation_flow_conflict` is an **informational caveat only** (it never holds back the bias — a momentum-driven `ADD_BIAS` can coexist with a cheap-but-being-sold caveat); whether an open valuation↔flow conflict should hard-suppress the call (force `NEUTRAL`/`NO_CALL`) is deferred until the forward eval shows whether conflicted ADD_BIAS calls actually underperform. **Flow-reversal sign-agreement guard** is the third veto-class item deferred here: this spec keeps the `0.4·5d + 0.6·20d` blend and shows 5d & 20d side-by-side on the board (a reversal is *visible*), but does NOT let a 5d-vs-20d sign split down-weight the score — that guard is judged with the others against forward evidence.
