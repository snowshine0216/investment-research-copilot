...
|---|---|---|
| `DEEPSEEK_API_KEY` | Required for `irc run` (DeepSeek tasks) | Used by research synthesis, scoring rationales, thesis checks, and interactive Q&A. Validated at the call edge — not required at `Settings()` construction; `irc monitor` does not need it. |
| `MINIMAX_API_KEY`, `MINIMAX_BASE_URL`, `MINIMAX_MODEL` | Required for `irc monitor` | MiniMax provider for monitor tasks (`monitor_impact`, `monitor_narrative`). Validated at the call edge; legacy `irc run` tasks do not need these. **Use a fast non-reasoning chat model** (e.g. `MiniMax-Text-01`) — reasoning models (e.g. `MiniMax-M3`) overrun the per-call deadline and degrade briefs to `NO_CALL`. |
...
- LLM provider calls — DeepSeek (`api.deepseek.com`), OpenRouter (`openrouter.ai`), and MiniMax (url from `MINIMAX_BASE_URL`), driven by `config/llm.yaml`.
...
| `com.irc.flow-capture` | Daily 15:45, after the 15:00 close | `irc monitor flow-capture` + `notify-status --run-kind flow-capture` — silent when ok; pages on degradation/abstain/failure + a one-time abstain→ok recovery notice (data-health-notify) |
