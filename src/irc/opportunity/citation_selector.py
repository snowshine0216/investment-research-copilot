...
(D0e), memo evidence-pool (D1a), and discipline-renderer nested bullets (D3a).
...
Canonical location: `irc.opportunity.citation_selector`. The old
`irc.memo.citation_selector` import path is preserved as a re-export shim so
existing callers (memo internals, tests) keep working unchanged — the move
breaks an `opportunity → memo` cycle introduced when item 007 wired the
SAME-3 selector into `opportunity.report._render_section`.
